            ...
            {categories.map((item, index) => (
              ...
              >
                <Link href={`/collection#${item.slug}`} key={item.id}>
                  <a className="align-items-center font-color-black flex-column cursor-pointer mb-5" data-artemis="spa-navigation-click" onClick={() => this.props.dispatchTrackSelectPromotion(item.id, item.name, item.meta.image, index+1, this.props.location)}>
                    <div>
                      { item.meta?.image && (
                        <div
